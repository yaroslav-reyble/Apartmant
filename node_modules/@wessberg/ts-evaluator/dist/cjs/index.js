...
function evaluateSimpleLiteral(node, typescript) {
    ...
    else if (typescript.isBigIntLiteral(node))
...
function evaluateIdentifier(options) {
    ...
    if (valueDeclaration != null) {
        if (valueDeclaration.getSourceFile().isDeclarationFile) {
            ...
            logger.logBinding(node.text, implementation, `Discovered declaration value${valueDeclaration.getSourceFile() === node.getSourceFile()
                ? ""
        ...
        if (typescript.isVariableDeclaration(valueDeclaration) &&
        ...
        logger.logBinding(node.text, stackValue, `Discovered declaration value${valueDeclaration.getSourceFile() === node.getSourceFile()
            ? ""
            : ` (imported into '${node.getSourceFile().fileName}' from '${valueDeclaration.getSourceFile().fileName}')`}`);
        return stackValue;
    }
 ...
 */
function evaluate({ typeChecker, node, environment: { preset = exports.EnvironmentPresetKind.NODE, extra = {} } = {}, typescript = TSModule, logLevel = 0 /* SILENT */, policy: { deterministic = false, network = false, console = false, maxOps = Infinity, maxOpDuration = Infinity, io = {
    read: true,
    write: false
}, process = {
    exit: false,
